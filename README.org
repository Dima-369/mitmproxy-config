...
It means that =GET v3/projects?foo=bar&filter[editedOn][gte]=2023-06-05T09:52:00Z= is equivalent to =GET v3/projects?foo=bar=. The =filter[editedOn][gte]= parameter is simply ignored regardless what is contains for mapping the flow to the local file system.
